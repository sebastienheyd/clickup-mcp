...
test("addComment posts the comment and reports the failure when an image is not an image", async (t) => {
  const h = await setupHarness();
  ...
  assert.ok(
    !commentBody.comment.some((b: any) => b.type === "image"),
    "no image fragment should be produced"
  );
  // The caption must survive so the reader knows something was meant to be there
  assert.ok(
    commentBody.comment.some((b: any) => b.text === "Kaputtes Bild"),
    "caption should be kept as text instead of being dropped"
  );
  assert.ok(commentBody.comment.some((b: any) => b.text === "Text davor"));
  assert.ok(
    result.content[0].text.includes("WARNING"),
...
test("addComment rejects a missing file but still writes the comment", async (t) => {
  const h = await setupHarness();
  ...
  assert.ok(commentBody, "comment should have been posted");
